...
public class TicTacToeA
{
	...
	public static void main(String[] args)
   {
		...
		while(true)
		{
			...
			gameResult = checkGameStatus(tic);
			switch (gameResult)
			{
				case 1:
	...
	private static byte checkGameStatus(TicTacToeA tic)
	{
		boolean draw = true;
		...
		for (byte row = 0; row <= 2; row++)
		{
			for (byte column = 0; column <= 2; column++)
			{
				...
				else if (tic.table[row][column] == Value.O)
				{
					player2Win = false;
				}
				else if (tic.table[row][column] == Value.X)
				{
					player1Win = false;
				}
			}
			if (player1Win)
				return tic.PLAYER1WIN;
			else if (player2Win)
				return tic.PLAYER2WIN;
		...
		// Checking up to bottom diagonal
		for (byte counter = 0; counter <= 2; counter++)
		{
			if (tic.table[counter][counter] == Value.EMPTY)
			{
				player1Win = false;
				player2Win = false;
				break;
			}
			else if (tic.table[counter][counter] == Value.O)
			{
				player2Win = false;
			}
			else if (tic.table[counter][counter] == Value.X)
			{
				player1Win = false;
			}
		}
		...
		// Checking bottom to up diagonal
